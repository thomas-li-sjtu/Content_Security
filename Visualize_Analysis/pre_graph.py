...
def Match(comment, content):
    ...
    for k in range(0, len(content)):
        ...
        if "True" not in judge:
            ...
            nodes1[content[k]['user_id']]['symbolSize'] = nodes1[content[k]['user_id']].get('symbolSize',
                                                                                            0) + symbolSize  # symbolSize① 被别人评论
            nodes1[content[k]['user_id']]['value'] = nodes1[content[k]['user_id']].get('value',
                                                                                       0) + value  # value①  被别人评论
            ...
            for i in comment:
                if i['weibo_url'] == url:
                    ...
                    nodes1[i["comment_user_id"]]["symbolSize"] = nodes1[i["comment_user_id"]].get('symbolSize',
                                                                                                  0) + 0.2  # symbolSize② 评论别人
                    nodes1[i["comment_user_id"]]["value"] = nodes1[i["comment_user_id"]].get("value",
                                                                                             0) + 1  # value② 评论别人
