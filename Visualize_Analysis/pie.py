...
keyword = ['disease', 'trump'][1]

df_weibo = pd.read_csv(f'./result/Senti_{keyword}.csv', sep=',', encoding='gb18030')
...
senti_median = median(senti_list)
...
for i in senti_list:
    if i < -4:
        senti_dict['Extremely-Negative'] = senti_dict.get("Extremely-Negative", 0) + 1
    elif -4 <= i < 1:
        ...
        senti_dict['Extremely-Positive'] = senti_dict.get("Extremely-Positive", 0) + 1
...
#         Pie()
#         .add("", data)
#         .set_global_opts(title_opts=opts.TitleOpts(title=f"Pie-{keyword}"))
...
# Page().add(*[fn() for fn, _ in C.charts]).render(f'./result/pie-{keyword}.html')
